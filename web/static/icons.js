// Inline SVG icons.
//
// The mockup pulls Font Awesome from a CDN. This binary serves every byte it
// needs, so the handful of glyphs actually used are inlined here instead — no
// request, no 900KB icon font, and they inherit currentColor for free.
...
const P = {
  ...
  cap: '<path d="m22 10-10-5L2 10l10 5 10-5z"/><path d="M6 12v5c0 1 2.7 2.5 6 2.5s6-1.5 6-2.5v-5"/>',
...
export function icon(name) {
  const body = P[name] || P.chat;
  return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
    stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${body}</svg>`;
}

// paint fills every <span class="ico" data-icon="…"> inside a root.
export function paintIcons(root = document) {
  for (const el of root.querySelectorAll(".ico[data-icon]")) {
