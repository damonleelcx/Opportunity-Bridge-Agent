...
func (w *WebSearch) Lookup(ctx context.Context, q Query) ([]Result, error) {
	...
	// Steered at official and public-service sources rather than at the open web:
	// this product sends people to counters, and a random aggregator's listing is
	// not something it should be putting in front of somebody.
	terms := strings.TrimSpace(q.Keyword)
	if terms == "" {
		terms = "招聘 培训"
	}
	query := fmt.Sprintf("%s %s 公共就业服务 OR 人力资源和社会保障 OR 公共招聘", city, terms)
	...
	for _, r := range parsed.Web.Results {
		...
		out = append(out, Result{
			...
			Title: r.Title, Summary: r.Description, URL: r.URL,
			Source: "网络检索结果（未经核实）",
			Caveat: "这是网上搜到的线索，不是本系统核实过的岗位。点开看发布单位和日期，" +
				"拿不准就打 12333 核一下再跑一趟。",
		})
		if len(out) >= limit {
			break
